...
class FEWDomain(nn.Module):
    def __init__(self, layers=101, classes=2, zoom_factor=8, \
       ...
       ################yzg vgg -frm based 
        ...
        # # self.deeplabv1head = nn.Sequential(
        # #     nn.Conv2d(feat_dim, 1024, kernel_size=3, padding=12, dilation=12),
        # #     #nn.GroupNorm(8,reduce_dim),
        # #     nn.ReLU(inplace=True),
        # #     nn.Dropout2d(p=0.5) ,
        # #     nn.Conv2d(1024, 1024, kernel_size=1, padding=0),    
        # #    # nn.GroupNorm(8,reduce_dim),
        # #     nn.ReLU(inplace=True),
        # #     nn.Dropout2d(p=0.5),
        # #     nn.Conv2d(1024, 19, kernel_size=1, padding=0)
        # # )  
   ...
   ############forward for sourceonly or targetonly
    ...
    # def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), train_label_list=None, y=None, epoch=None):
    #     x_size = x.size()
    #     #print(x.shape)
    #     #assert (x_size[2]-1) % 8 == 0 and (x_size[3]-1) % 8 == 0
    #     #h = int((x_size[2] - 1) / 8 * self.zoom_factor + 1)
    ...
    #         #with torch.no_grad():
    #         supp_feat_0 = self.layer0(s_x[:,shot,:,:,:])
    #         supp_feat_1 = self.layer1(supp_feat_0)
    #         supp_feat_2 = self.layer2(supp_feat_1)
    #         supp_feat_3 = self.layer3(supp_feat_2)
    ...
    #           #print(k,c)
    #           mask = (s_y[:,shot,:,:] == c.cuda()).float().unsqueeze(1)
    ...
    #           if shot>0:                  #for the non-first support images, just add the prototype feature to history prototypes
    #               prototype_list[k] = prototype_list[k] + prototype
    #           else:
    ...
    #            prototype_list[c] = prototype_list[c] / prototype_num_eachclass[c]
    #            print(prototype_num_eachclass[c])
    ...
    #     # q_prototype_num_eachclass = [j-j for j in range(n_classes)]
    #     # q_prototype_list= []
    #     # #q_prototype_list_temp= []
    #     # for k, c in enumerate(train_label_list):#range(n_classes):
    #     #       #print('ccccccccccccccccccc',c,train_label_list)
    #     #       #print(k,c)
    #     #       mask = (y[:,:,:] == c.cuda()).float().unsqueeze(1)
              
    #     #       #print('mask.shape,supp_feat.shape',mask.shape,supp_feat.shape)
    #     #       #mask_list.append(mask)
    #     #       mask = F.interpolate(mask, size=(query_feat.size(2), query_feat.size(3)), mode='bilinear', align_corners=True)
    #     #       #print('mask.shape after interpolation',np.unique(mask.cpu().numpy()))# 降采样后 MASK中有些小目标可能干没了
    #     #       q_prototype = Weighted_GAP(query_feat, mask)#get protype for each class in train_label_list
    #     #       #print(prototype.shape) #torch.Size([1, 256, 1, 1])
    #     #       q_prototype = torch.squeeze(q_prototype)#.unsqueeze(0)  #256
    #     #       q_prototype_list.append(q_prototype)#n_clasess*256
    #     #       if 1 in np.unique(mask.cpu().numpy()): #in general, an image will not contribute prototype to all the classes
    #     #         q_prototype_num_eachclass[k] = q_prototype_num_eachclass[k]+1
    #             #q_prototype_list_temp.append(q_prototype)
    ...
    #     #     if    prototype_num_eachclass[k]==0: #q_prototype_num_eachclass[k]==0 or
    #     #         #pixelind_i = np.where((s_y.cpu().numpy()) == c.cpu().numpy())
    #     #         #s_y[pixelind_i[0],pixelind_i[1],pixelind_i[2],pixelind_i[3]] = 255
    #     #         #if k in disappear_label:
    #     #             print('in training disapper', c)
    #     #     else:
    #     #            #disappear_label.append(k) 
    #     #            prototype_list_temp.append(prototype_list[k])
    #     #            q_prototype_list_temp.append(q_prototype_list[k])
    #             #prototype_list.pop(k)
                     
                          
    #     #print(disappear_label)


  

    #     # prototype_tensor = torch.stack(prototype_list_temp,dim=0)
    #     # q_prototype_tensor = torch.stack(q_prototype_list_temp,dim=0)
    #     #print(len(train_label_list))

    #     # print(q_prototype_tensor.shape,prototype_tensor.size(0))
    #     # print(prototype_tensor.shape)
